...
class CkModel:
    ...
    def __init__(self, model_settings):
        ...
        self.cell_size  = model_settings["cell_size"]
        ...
        tf.add_to_collection('configs', tf.constant(self.cell_size, name="cell_size"))
    ...
    def _create_placeholders(self):
        ...
        with tf.name_scope("placeholder"):       
            ...
            #self.fw_state = tf.placeholder(tf.float32, [self.num_layers, 2, None, self.cell_size], name="fw_state")
            #self.bw_state = tf.placeholder(tf.float32, [self.num_layers, 2, None, self.cell_size], name="bw_state")        
    ...
    def _inference(self):
        ...
        for i in range(0, self.num_layers):
            if (self.cell_type == 'lstm'):
                cell = rnn.LSTMCell(num_units=self.cell_size, state_is_tuple=True)                
            elif (self.cell_type == 'gru'):
                # change to GRU
                cell = rnn.LSTMCell(num_units=self.cell_size, state_is_tuple=True)
            else:
                cell = rnn.BasicRNNCell(num_units=self.cell_size)
